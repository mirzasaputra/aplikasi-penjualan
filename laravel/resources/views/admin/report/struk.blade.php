...
<head>
    ...
    <link rel="shortcut icon" href="{{ url('/admin/uploads/img/' .getSetting('favicon')) }}">
    <!-- StyleSheets  -->
    <link rel="stylesheet" href="{{ url('/admin/assets/assets/css/bootstrap.min.css') }}">
    <style>
        .baris {
            display: block;
            width: 100%;
...
<body>
    <div class="baris">
        <div class="kolom-1">
            <img src="{{ url('/admin/uploads/img/'. getSetting('logo')) }}" width="100%" alt="">
        </div>
    ...
    <div class="mb-4" style="font-size: 12px;">
        <table width="40%">
            ...
            <tr>
                ...
                @php
                    $date = date_create($collection->tgl);
                @endphp
        ...
        <table class="table table-bordered" width="100%">
            ...
            @php
                $total = 0;
            @endphp
            @foreach($collection->penjualan_detail as $detail)
                @php
            ...
            <tr>
                ...
                <th>Rp. {{ number_format($total * $collection->diskon / 100, 0, ',', '.') }}</th>
            </tr>
            <tr>
                <th colspan="2"></th>
                <th colspan="2">Total</th>
                <th>Rp. {{ number_format($collection->total, 0, ',', '.') }}</th>
            </tr>
            <tr>
                <th colspan="2"></th>
                <th colspan="2">Bayar</th>
                <th>Rp. {{ number_format($collection->bayar, 0, ',', '.') }}</th>
            </tr>
            <tr>
                <th colspan="2"></th>
                <th colspan="2">Kembali</th>
                <th>Rp. {{ number_format($collection->kembali, 0, ',', '.') }}</th>
            </tr>
        </table>
    </div>
</body>
